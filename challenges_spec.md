...
- **Display to User**: `{correct}/{total}` (e.g., “8/10” or “4/5”) — what they see on results screen
- **Ranking**: `total_score` (0-1000 scale) — what determines leaderboard position
- **User will NOT see raw score** — only the display format
...
1. **Always rank by `total_score`**, never by `correct_count`
1. **Always display as `{correct}/{total}`**, never show raw score to user
1. **Ties possible**: Two players with same `total_score` → show in order by `submitted_at` (earliest first)
1. **Speed matters**: This is by design — it rewards quick, accurate answers

-----

## ⚠️ CRITICAL: Score-Based Ranking Rule

**This is the most important rule for Open Challenges:**

### ALL leaderboard rankings are determined by `total_score`, NOT by `correct_count`

|Aspect             |Rule                                                                       |
|-------------------|---------------------------------------------------------------------------|
|**Ranking Logic**  |Sort by `total_score DESC, submitted_at ASC`                               |
|**Display Format** |Show as `{correct_count}/{total_questions}` (e.g., “9/10”)                 |
|**User Visibility**|Users see `{correct}/{total}`, NOT `total_score`                           |
|**Comparison**     |Two players with same display score (e.g., “9/10”) can have different ranks|
|**Reason**         |Speed matters — a fast 8/10 ranks higher than slow 9/10                    |

### Examples

**Example 1: Same Correctness, Different Speeds**

```
Display: Both show "9/10"
Behind scenes:
  Player A: total_score = 1450 (fast) → Rank #1
  Player B: total_score = 1200 (slow) → Rank #2
```

**Example 2: Showing in Leaderboard (What User Sees)**

```
🥇 @alice (9/10)     ← total_score: 1450
🥈 @bob (9/10)       ← total_score: 1200
🥉 @carol (8/10)     ← total_score: 1300

Even though @carol has lower "correct", they rank #3 due to speed!
```
...
│ High Score: 1200                 │ ← Display: correct/total
│ Your Best: 950 (if played)       │ ← Display: correct/total
│                                   │
│ Leaderboard (Top 5):              │
│ 🥇 @trivia_king (1200)           │ ← Ranked by total_score
│ 🥈 @quiz_master (1100)            │ ← (speed affects ranking)
│ 🥉 @sumit (1000)                  │ ← Same correctness ≠ same rank
...
│ Correct: 10/10               │ ← For context only
│ Time: 120 seconds            │
│                              │
